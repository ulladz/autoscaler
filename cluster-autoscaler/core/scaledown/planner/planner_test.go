...
func TestNodesToDelete(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			removableNodes: map[cloudprovider.NodeGroup][]simulator.NodeToBeRemoved{
				sizedNodeGroup("test-ng", 3, false): {
					buildRemovableNodeWithOnCompletionPods("test-node-active", 0, false),
				},
			},
			wantEmpty: []*apiv1.Node{},
			wantDrain: []*apiv1.Node{},
		},
		{
			...
			removableNodes: map[cloudprovider.NodeGroup][]simulator.NodeToBeRemoved{
				sizedNodeGroup("test-ng", 3, false): {
					buildRemovableNodeWithOnCompletionPods("test-node-terminal", 1, true),
				},
			},
			wantEmpty: []*apiv1.Node{},
			wantDrain: []*apiv1.Node{
				buildRemovableNodeWithOnCompletionPods("test-node-terminal", 1, true).Node,
			},
		},
		{
			name: "single empty",
...
func buildRemovableNodeWithOnCompletionPods(name string, podCount int, terminal bool) simulator.NodeToBeRemoved {
	node := buildRemovableNode(name, podCount)
	pod := &apiv1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:      fmt.Sprintf("%s-pod-%d", name, podCount),
			Namespace: "default",
